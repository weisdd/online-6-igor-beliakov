...
Создать класс , который представляет топологию сети.
...
class Topology:
    def __init__(self, duplicated_topology):
        filtered_topology = {}
        for local, remote in duplicated_topology.items():
            if remote not in filtered_topology:
                filtered_topology[local] = remote
        self.topology = filtered_topology
